...
def extract_years_from_urls(urls: List[str]) -> str:
    ...
    if years:
        return f"players_{min(years)}-{max(years)}.csv"
...
def save_to_csv(data: List[Dict[str, str]], filename: str) -> None:
    with open(filename, mode="w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["Name", "Position", "Club", "Nation", "Appearances", "Yellow Card Suspensions", "Yellow Cards", "Second Yellow Cards", "Red Cards", "Sending Offs", "Points", "Cards Per Match", "Year"])
        for player in data:
            writer.writerow([player["name"], player["position"], player["club"], player["nation"], player["appearances"], player["yellow_suspensions"], player["yellow_cards"], player["second_yellow_cards"], player["red_cards"], player["sending_offs"], player["points"], player["cards_per_match"], player["year"]])
...
if __name__ == "__main__":
    ...
    print(f"🎉 Data berhasil disimpan ke {csv_filename}! Total pemain unik: {len(players)}")
